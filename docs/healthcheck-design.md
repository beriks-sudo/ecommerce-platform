...
Endpoint `public/health.php` проверяет, что приложение реально может работать со
своими зависимостями (а не просто что порт открыт):
- MySQL — выполняет запрос `SELECT 1` через PDO (`pdo_mysql` ставится в Dockerfile);
- Redis — посылает `PING` и проверяет ответ `+PONG`.
- expected success: оба запроса успешны → health.php отдаёт 200 → curl
  возвращает 0 → healthy.
- expected failure: app ещё не готов или БД/Redis не отвечают на запрос →
  health.php отдаёт 503 → `curl -f` возвращает ненулевой код → неудача
  (копится retries).
Это «полезная» readiness: healthy означает, что app реально выполняет запрос к
БД и кэшу, а не просто что процесс существует или порт принимает соединение.
